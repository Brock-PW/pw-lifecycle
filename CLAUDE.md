...
## Task Priority Tiers (for dashboard sequencing)

Every task is assigned one of four tiers. The dashboard surfaces tasks in this priority order:
...
Each card on the lifecycle site will link to its SOP. Clicking a card opens the SOP as a slide-in panel.

---

## Zoho Lead → Account/Contact Architecture
...
**The rule:** All projects in Zoho are assigned to the Account, not the Contact.
One Account can have multiple Contacts.

---

## Notion Client Brain — Naming Convention
...
On completed call — not on booked call, not on lead creation.
A booked call that doesn't happen does not create a client brain page.

---

## Calendar & Otter Recording Rule

Every consultation call is scheduled via Google Calendar with a Google Meet link.
Whether it runs on Meet or flips to a direct call, it must always be recorded in Otter.ai.

**The Otter recording is the trigger for all post-call automation.**
No Otter recording = no automation fires.

**Calendar invite naming convention:** The invite must always contain the lead name
(e.g. "Jane Doe" or "Jane Doe — Larry's Construction" if company is known).
This is how Claude and Zoho connect the Otter transcript to the correct record.

---

## Email Sync — Full Team Required

The sync layer must connect Gmail for ALL client-facing team members.
Until all are connected, the client brain is incomplete for any thread not through Brock.

Priority order:
- **Brock** — already connected via MCP ✅
- **Anthy** — account manager, all client day-to-day communication — HOOK/COWORK then HOOK/SYNC · HIGH PRIORITY
- **Rachel** — billing and compliance threads — HOOK/COWORK then HOOK/SYNC
- **Megan** — industry relationships, consultation calls — HOOK/COWORK then HOOK/SYNC
